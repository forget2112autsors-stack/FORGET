...
10. [x] **1C andazasidagi Debet/Kredit qoidalarini birlashtirish va 4010-6010 ni avtomatlashtirish:** ✅ Bajarildi (O'zbekiston BHMS 21 hisobvaraqlar rejasi bo'yicha yagona `BUXGALTERIYA_SCHYOTLAR` va `getDocProvodkalari` dvigateli joriy etildi. Faktura kirim, chiqim, bank, kassa va ish haqi jadvallarida Dt/Kt provodkalarini 1-klikda ko'rish tugmasi va interaktiv modali `openProvodkaModal` ulandi).
...
12. [x] **Ish paneli (Operativ boshqaruv / Dashboard) bilan integratsiya mezonlari:** ✅ Bajarildi (Dashboardda jonli O'zbekiston Soliq Taqvimi — JShDS, Ijtimoiy soliq, QQS va Foyda solig'i to'lov muddatlari va hisoblangan summalari, 30+ kunlik muddati o'tgan debitorlik va kreditorlik nazorati, Tabel holati to'liq integratsiya qilindi).
13. [x] **Desktop va Mobile build mosligi:** ✅ Bajarildi (`build-desktop.ps1` va `build-mobile.ps1` ga `kg1_data.js` va `kg1_mahsulotlar.json` to'liq ulandi, staging kataloglari sinxronlashtirildi).
